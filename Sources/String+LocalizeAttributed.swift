...
public extension String {
	...
	private func parse(mutable: NSMutableAttributedString) -> NSMutableAttributedString {
		...
		let matches : [NSTextCheckingResult] = regexp.matches(in: mutable.string, options: [], range: NSMakeRange(0, mutable.string.count))
	...
	func parse(style: String) -> [NSAttributedStringKey:Any] {
		...
		let matches : [NSTextCheckingResult] = regexp.matches(in: style, options: [], range: NSMakeRange(0, style.count))
		...
		for match in matches {
            ...
            if let temp = self.attribute(key: key, value: value) {
